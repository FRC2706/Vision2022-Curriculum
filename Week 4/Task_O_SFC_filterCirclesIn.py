...
def filterCirclesIn(bgrImage, mskImage, contours, cv2v):
    ...
    for indiv in contours:
        ...
        if (0.75 < brextent < 0.85) and (0.75 < arextent< 0.85): 
            circularContours.append(indiv)
            #print(f'contour area={area}, width={brw}, height={brh}, brarea={brw*brh}')
            #print('indiv=', intCounter, 'brextent=', brextent)
            #print(f'contour area={area}, width={arw}, height={arh}, brarea={arw*arh}')
            #print('indiv=', intCounter, 'arextent=', arextent)
    ...
    cv2.drawContours(bgrImage, circularContours, -1, colBgrOrange, 2)

    # create a new mask with only the desired contours
    mskBinaryCircles = np.zeros(shape=[height, width, 1], dtype=np.uint8)
    cv2.drawContours(mskBinaryCircles, circularContours, -1, 255, -1)

    # create a full color mask
    # Bitwise-AND binary mask and original image
    mskColor = cv2.bitwise_and(bgrImage, bgrImage, mask=mskBinaryCircles)

    if circularContours:

        # sort contours by area decreasing
        #sortedContours = sorted(circularContours, key=lambda x: cv2.contourArea(x), reverse=True)

        # sort contours by area increasing
        #sortedContours = sorted(circularContours, key=lambda x: cv2.contourArea(x), reverse=True)

        # sort contours by leftmost point of contour
        sortedContours = sorted(circularContours, key=lambda x: tuple(x[x[:,:,0].argmin()][0])[0], reverse=False)
...
if __name__ == "__main__":
    ...
    mskBinary = cv2.inRange(hsvTestImage,  (25, 220, 220), (35, 255, 255),)
